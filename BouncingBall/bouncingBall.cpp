...
TTF_Font* font;
//分數文字
int scoreFontReslution = 10; //字體解析度
SDL_Color scoreColor = { 255, 255, 255, 50 }; //字體顏色 {red, green, blue ,alpha}
SDL_Rect scoreRect = { 540, 500, 40, 80 }; //字體尺寸 {起始x, 起始y, width, height}

//統計文字
int statisticsFontReslution = 15; //字體解析度
SDL_Color statisticsTextColor = { 255, 255, 150, 0 }; //字體顏色
SDL_Rect statisticsTextRect = { 100, 100, 300, 100 }; //字體尺寸

//統計分數
int statisticsScoreReslution = 10; //字體解析度
...
int main(int argc, char* argv[])
{
    ...
    while (window)
    {
        ...
        if (!pause) {
            if (!UpdateLogic())
            {
                ...
                cout << "Your score is: " << score/2 << std::endl;
                SDL_DestroyWindow(window);
                window = NULL;
            }
        }
    ...
    while (window) {
        ...
        // 繪製統計分數
        SDL_Surface* statisticScoreSurface = TTF_RenderText_Solid(font, std::to_string(score / 2).c_str(), statisticsScoreColor);
...
void draw(void)
{
    ...
    SDL_Surface* scoreSurface = TTF_RenderText_Solid(font, std::to_string(score/2).c_str(), scoreColor);
...
void init()
{
    ...
    //載入文字
    font = TTF_OpenFont("C:/Project/ttf/sans.ttf", scoreFontReslution);
    if (!font) {
        std::cout << "Font is not loaded, error shows below!" << std::endl;
